...
Минимальный путь:
- login (через AuthScreen),
- перейти в Catalog,
- открыть Room.
- Переходы выполняются по иконкам, чтобы тест не зависел от RU/EN локали.
- В тесте используются фейковые ApiClient/WsClient, чтобы сценарий был детерминирован и не требовал сеть.
...
Инструменты `k6`/`artillery` в проекте не используются (пометка: «не указано»).
