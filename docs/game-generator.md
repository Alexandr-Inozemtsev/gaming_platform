...
- `--generated-at` — опциональная фиксация метадаты времени для детерминированных коммитов.

Для фикстур в репозитории рекомендуется умеренный размер (`--n 30..100`), чтобы не раздувать diff.
