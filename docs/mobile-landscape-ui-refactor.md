...
## Mockup parity gate (обязательный для UI-задач)

1. **Блокирующее условие завершения**
   - UI-задача не может считаться завершённой без сравнения `mockup vs build`.
   - Сравнение выполняется только на одинаковых viewport и одинаковых состояниях сцены.

2. **Минимальный набор состояний Big Walker для parity-сверки**
   - `idle`
   - `roll`
   - `next_turn`
   - `pause`
...
3. **Acceptance-критерии parity**
   - Сохранена многослойная композиция (фон комнаты + атмосфера + стол/борд + HUD + action panel + overlay).
   - HUD остаётся читаемым в mobile landscape без потери контраста.
   - Active player визуально очевиден.
   - Action panel визуально согласована с mockup и не выглядит как placeholder.
