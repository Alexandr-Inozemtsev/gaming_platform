...
> Важно для PR из Codex-среды: бинарные файлы в PR не поддерживаются. Поэтому `reference_screens/*.png` используются только локально для сверки и не должны быть runtime-зависимостью игрового экрана.
...
6. при необходимости подключать полноэкранные референсы только локально для разработки и сверки (не как обязательный runtime слой).
